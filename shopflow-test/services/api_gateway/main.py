...
# Services
from services.auth_service.main import (
    app as auth_app,
    LoginRequest,
    login as auth_login,
    get_demo_users,
...
from services.product_api.main import (
    app as product_app,
    list_products,
    get_product,
    get_categories
)
from services.order_api.main import (
    app as order_app,
    CreateOrderRequest,
    list_orders,
    get_order,
...
from services.checkout_api.main import (
    app as checkout_app,
    CartRequest,
    CheckoutRequest,
    update_cart,
...
async def telemetry_middleware(request: Request, call_next):
    ...
    # Check for simulated gateway latency injection
    chaos_status = chaos_engine.get_status()
    if chaos_status.get("active_scenario") == "high_latency" and chaos_status.get("state") == "RUNNING":
        time.sleep(0.15)
    ...
    if not path.startswith("/telemetry") and not path.startswith("/assets") and not path.startswith("/api/chaos/status"):
...
def api_login(request: LoginRequest):
    return auth_login(request=request)
